...
<main>      <div class="form-container">
            <h2>Add New Tournament Official</h2>
            <form action="tournamentOfficials.php" method="POST" class="form">
                ...
                <textarea id="background" name="background" rows="4" required></textarea><br><br>
                <label for="matchID">Match ID:</label><br>
                <input type="number" id="matchID" name="matchID" required><br><br>
                <input type="submit" value="Submit">
                <input type="button" value="Cancel" onclick="window.location.href='tournamentOfficials.php'">
            </form>
        </div>
    </div>
    ...
    // Check if the submitted Match ID exists in the Matches table
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $matchID = $_POST['matchID'];

        // Prepare and execute the query
        $checkMatchQuery = "SELECT * FROM Matches WHERE Match_ID = ?";
        $stmt = $conn->prepare($checkMatchQuery);
        $stmt->bind_param("i", $matchID);
        $stmt->execute();
        $result = $stmt->get_result();

        // If Match ID exists, proceed with inserting into TournamentOfficials table
        if ($result->num_rows > 0) {
            ...
            $insertQuery = "INSERT INTO TournamentOfficials (Match_ID, Name, Position, Nationality, Age, Background) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($insertQuery);
            $stmt->bind_param("isssis", $matchID, $name, $position, $nationality, $age, $background);
            ...
            echo "Error: Match ID does not exist";
        }
